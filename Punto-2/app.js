class Task {
  constructor(id, description, completed = false, important = false) {
    this.id = id;
...
class TaskManager {
  ...
  renderTasks() {
    ...
    this.tasks.forEach((task) => {
      ...
      const importanttask = document.createElement("button");
      importanttask.textContent = task.important
        ? "No importante"
        : "Importante";
        importanttask.addEventListener("click", (e) => {
          e.stopPropagation();
          this.importanttask(task.id);
        });
        

      item.appendChild(editButton);
      item.appendChild(importanttask);
